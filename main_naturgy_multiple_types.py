#!/usr/bin/env python3
"""
Script para extraer múltiples tipos de issues del proyecto Darwin (DAR) en Jira Naturgy-ADN
Extrae: Incidencia, Soporte sin PaP, Defecto
"""
...
class NaturgyJiraExtractorMultiple:
    """Extractor de múltiples tipos de issues de Jira para Naturgy-ADN"""
    ...
    def __init__(self):
        ...
        # Usar un directorio diferente para no sobrescribir la extracción anterior
        self.output_dir = Path("EXTRACCION_NATURGY_DAR_COMPLETA")
    ...
    def search_issues(self, start_at=0, max_results=100):
        """
        Busca issues de múltiples tipos en el proyecto DAR
        """
        url = f"{self.base_url}/rest/api/3/search/jql"
        
        # JQL para buscar múltiples tipos de issues
        # Incluye: Incidencia, Soporte sin PaP, Defecto
        jql = f'project = {PROJECT_KEY} AND issuetype IN ("Incidencia", "Soporte sin PaP", "Defecto") ORDER BY created DESC'
    ...
    def save_issue_markdown(self, issue_data):
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            f.write(f"- **Tipo:** {issue_data['issue_type']}\n")
    ...
    def extract_all_issues(self):
        ...
        print("=" * 80)
        print("🚀 EXTRACTOR MÚLTIPLE - NATURGY-ADN DARWIN (DAR)")
        print("=" * 80)
        print(f"📍 Proyecto: {PROJECT_KEY}")
        print(f"📋 Tipos de issue: Incidencia, Soporte sin PaP, Defecto")
        ...
        print(f"\n📋 Desglose por tipo:")
        for issue_type, count in sorted(issue_types_count.items()):
        ...
        self.create_summary(total_extracted, issue_types_count)
    
    def create_summary(self, total_extracted, issue_types_count):
        """Crea un archivo de resumen de la extracción"""
        summary_file = self.output_dir / "RESUMEN_EXTRACCION.md"
        
        with open(summary_file, 'w', encoding='utf-8') as f:
            f.write("# Resumen de Extracción - Naturgy-ADN Darwin (Completa)\n\n")
            f.write(f"**Fecha de extracción:** {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}\n\n")
            f.write(f"**Proyecto:** {PROJECT_KEY}\n")
            f.write(f"**Tipos de issue:** Incidencia, Soporte sin PaP, Defecto\n")
...
def main():
    """Función principal"""
    try:
        extractor = NaturgyJiraExtractorMultiple()
